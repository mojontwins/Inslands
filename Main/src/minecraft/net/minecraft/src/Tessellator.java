		...
		if(!this.isDrawing) {
			...
			if(!this.renderingChunk) {
				...
				if(this.useVBO) {
					...
					ARBVertexBufferObject.glBindBufferARB(GL15.GL_STATIC_DRAW, this.vertexBuffers.get(this.vboIndex));
					ARBVertexBufferObject.glBufferDataARB(GL15.GL_STATIC_DRAW, this.byteBuffer, GL15.GL_STREAM_DRAW);
		...
		if(!this.renderingChunk) {
			...
			if(this.drawMode == 7 && convertQuadsToTriangles && this.addedVertices % 4 == 0) {
				for(int i = 0; i < 2; ++i) {
					...
					//this.intBuffer.put(this.intBuffer.get(this.rawBufferIndex - j + 7));
