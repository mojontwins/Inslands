...
public class EntityEgg extends Entity {
	...
	public int shake = 0;
	private EntityLiving throwerEntity;
	private int age;
	...
	public EntityEgg(World world1, EntityLiving entityLiving2) {
		super(world1);
		this.throwerEntity = entityLiving2;
		this.setSize(0.25F, 0.25F);
	...
	public void onUpdate() {
		...
		if(!this.worldObj.isRemote) {
			...
			for(int i8 = 0; i8 < list5.size(); ++i8) {
				...
				if(entity9.canBeCollidedWith() && (entity9 != this.throwerEntity || this.ticksOnAir >= 5)) {
					float f10 = 0.3F;
		...
		if(movingObjectPosition3 != null) {
			if(movingObjectPosition3.entityHit != null && movingObjectPosition3.entityHit.attackEntityFrom(this.throwerEntity, 0)) {
				;
			}
	...
	public void onCollideWithPlayer(EntityPlayer entityPlayer1) {
		if(this.inGround && this.throwerEntity == entityPlayer1 && this.shake <= 0 && entityPlayer1.inventory.addItemStackToInventory(new ItemStack(Item.arrow, 1))) {
