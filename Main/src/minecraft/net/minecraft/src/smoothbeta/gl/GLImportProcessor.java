			...
			if (method_36424(source, matcher, j)) {
				...
				if (string2 != null) {
					...
					if (!StringUtils.isNullOrEmpty(string5)) {
						if (!StringUtils.endsWithLineBreak(string5)) {
							string5 = string5 + System.lineSeparator();
						}

						++context.line;
						k = context.line;
						List<String> list2 = this.parseImports(string5, context, bl ? string4 : "");
					...
					k = StringUtils.countLines(source.substring(0, matcher.end(1)));
