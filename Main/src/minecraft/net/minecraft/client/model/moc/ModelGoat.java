package net.minecraft.client.model.moc;
...
public class ModelGoat extends ModelBase {
	...
	@Override
	public void render(float f, float f1, float f2, float f3, float f4, float partialTick) {
		super.render(f, f1, f2, f3, f4, partialTick);
		this.setRotationAngles(f, f1, f2, f3, f4, partialTick);
		this.Leg1.render(partialTick);
		this.Leg2.render(partialTick);
		this.Leg3.render(partialTick);
		this.Leg4.render(partialTick);
		this.Body.render(partialTick);
		this.Tail.render(partialTick);
		this.Neck.render(partialTick);
		if (this.typeInt > 1 && this.typeInt < 5) {
			this.Tits.render(partialTick);
		}
		GL11.glPushMatrix();

		if (this.attacking != 0) {
			// float yOff = partialTick*((2.0F*this.attacking)/45F)-1.33333F;
			// float zOff = partialTick*(this.attacking/9F)-3.33333F;

			// 0.3! yOff = (3F*this.attacking/450F) - (3F*3F/45F);
			// float yOff = 0.3F;
			// yOff = (3F*this.attacking/450F) - (3F*3F/45F);
			float yOff = (this.attacking / 150F) - (1F / 5F);
			// float zOff = 0.1F;
			float zOff = (this.attacking / 450F) - (1F / 15F);
			GL11.glTranslatef(0.0F, yOff, -zOff);
		}
		this.LEar.render(partialTick);
		this.REar.render(partialTick);
		this.Head.render(partialTick);
		this.Nose.render(partialTick);

		if (this.typeInt > 1) {
			if (this.edad > 0.7) {
				this.RHorn1.render(partialTick);
				this.LHorn1.render(partialTick);
			}
			if (this.edad > 0.8) {
				this.RHorn2.render(partialTick);
				this.LHorn2.render(partialTick);
			}
		}
		if (this.typeInt > 4) {
			if (this.edad > 0.8) {
				this.RHorn3.render(partialTick);
				this.LHorn3.render(partialTick);
			}
			if (this.edad > 0.85) {
				this.RHorn4.render(partialTick);
				this.LHorn4.render(partialTick);
			}
			if (this.edad > 0.9) {
				this.RHorn5.render(partialTick);
				this.LHorn5.render(partialTick);

				// Goatie.render(partialTick);
			}
		}
		// mouth movement
		...
		if (this.typeInt > 4 && this.edad > 0.9) {
			this.Goatie.render(partialTick);
		}
		this.Tongue.render(partialTick);
		this.Mouth.render(partialTick);
