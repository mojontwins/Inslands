...
public class GuiDetailSettingsOF extends GuiScreen {
	...
	private static EnumOptions[] enumOptions = new EnumOptions[] { EnumOptions.CLOUDS,
			// EnumOptions.CLOUD_HEIGHT,
			EnumOptions.TREES, EnumOptions.GRASS, EnumOptions.WATER, EnumOptions.RAIN, EnumOptions.SKY,
			EnumOptions.STARS,
	...
	public void initGui() {
		...
		for (int k = 0; k < j; ++k) {
			...
			if (!enumoptions.getEnumFloat()) {
				this.controlList.add(new GuiSmallButton(enumoptions.returnEnumOrdinal(), x, y, enumoptions,
						this.settings.getKeyBinding(enumoptions)));
			} else {
				this.controlList.add(new GuiSlider(enumoptions.returnEnumOrdinal(), x, y, enumoptions,
						this.settings.getKeyBinding(enumoptions), this.settings.getOptionFloatValue(enumoptions)));
			}

			++i;
		}

		this.controlList.add(new GuiButton(200, this.width / 2 - 100, this.height / 6 + 168 + 11,
	...
	protected void actionPerformed(GuiButton guibutton) {
		if (guibutton.enabled) {
			...
			if (guibutton.id != EnumOptions.CLOUD_HEIGHT.ordinal()) {
				ScaledResolution scaledresolution = new ScaledResolution(this.mc.gameSettings, this.mc.displayWidth,
	...
	private String[] getTooltipLines(String btnName) {
		if (btnName.equals("Clouds")) {
			return new String[] { "Clouds", "  Default - as set by setting Graphics",
			...
			return new String[] { "Cloud Height", "  OFF - default height",
				"  100% - above world height limit" };
		} else if (btnName.equals("Trees")) {
			return new String[] { "Trees", "  Default - as set by setting Graphics", 
			...
			return new String[] { "Grass", "  Default - as set by setting Graphics", 
				"  Fast - lower quality, faster", 
				"  Fancy - higher quality, slower", 
			...
			return new String[] { "Autosave interval", 
			...
			return new String[] { "Fast Debug Info", 
			...
			return new String[] { "Chunk updates per frame", 
