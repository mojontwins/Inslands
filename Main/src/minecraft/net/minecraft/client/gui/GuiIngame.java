...
public class GuiIngame extends Gui {
	...
	public void renderGameOverlay(float f1, boolean z2, int i3, int i4) {
		ScaledResolution scaledResolution5 = new ScaledResolution(this.mc.gameSettings, this.mc.displayWidth,
		...
		if (this.mc.thePlayer.freezeLevel > 0.0D)
			this.renderFreezeFrame(brightness, i6, i7);
		...
		if (!this.mc.thePlayer.isStatusActive(Status.statusDizzy)) {
			float f10 = this.mc.thePlayer.prevTimeInPortal
					+ (this.mc.thePlayer.timeInPortal - this.mc.thePlayer.prevTimeInPortal) * f1;
			if (f10 > 0.0F) {
		...
		if (this.mc.gameSettings.showDebugInfo) {
			...
			fontRenderer8.drawStringWithShadow("Minecraft " + Version.getVersion() + " (" + this.mc.debug + ")", 2, 2,
					0xFFFFFF);
			...
			/*
			 * this.drawString(fontRenderer8, "x: " + this.mc.thePlayer.posX, 2, 64,
			 * 14737632); this.drawString(fontRenderer8, "y: " + this.mc.thePlayer.posY, 2,
			 * 72, 14737632); this.drawString(fontRenderer8, "z: " + this.mc.thePlayer.posZ,
			 * 2, 80, 14737632); this.drawString(fontRenderer8, "f: " +
			 * (MathHelper.floor_double((double)(this.mc.thePlayer.rotationYaw * 4.0F /
			 * 360.0F) + 0.5D) & 3), 2, 88, 14737632);
			 */

			fontRenderer8.drawStringWithShadow("Pos: " + (int) this.mc.thePlayer.posX + " "
					+ (int) this.mc.thePlayer.posY + " " + (int) this.mc.thePlayer.posZ + " ("
					+ (((int) this.mc.thePlayer.posX) >> 4) + " " + (((int) this.mc.thePlayer.posY) >> 4) + " "
					+ (((int) this.mc.thePlayer.posZ) >> 4) + ") [" + (int) this.mc.thePlayer.rotationYaw + "]", 2, 42,
					0xFFFFFF);

			float timeAdjusted = (float) (this.mc.theWorld.worldInfo.getWorldTime() % 24000);
			fontRenderer8.drawStringWithShadow("Time: " + this.twoDigits((int) ((timeAdjusted / 1000.0F) + 6) % 24)
					+ ":" + this.twoDigits((int) ((timeAdjusted % 1000.0F) * 60 / 1000)), 2, 52, 0xFFFFFF);
			...
			string21 = "Biome: " + this.mc.theWorld.getBiomeGenAt((int) this.mc.thePlayer.posX,
					(int) this.mc.thePlayer.posZ).biomeName;
		...
		if (this.onScreenMessageTimeout > 0) {
			...
			if (i16 > 0) {
				...
				fontRenderer8.drawString(this.onScreenMessage, -fontRenderer8.getStringWidth(this.onScreenMessage) / 2,
						-4, i17 + (i16 << 24));
				GL11.glDisable(GL11.GL_BLEND);
				GL11.glPopMatrix();
			}
	...
	private void renderVignette(float f1, int i2, int i3) {
		...
		this.prevVignetteBrightness = (float) ((double) this.prevVignetteBrightness
				+ (double) (f1 - this.prevVignetteBrightness) * 0.01D);
		GL11.glDisable(GL11.GL_DEPTH_TEST);
		GL11.glDepthMask(false);
		...
		tes.addVertexWithUV(0.0D, (double) i3, -90.0D, 0.0D, 1.0D);
		tes.addVertexWithUV((double) i2, (double) i3, -90.0D, 1.0D, 1.0D);
		tes.addVertexWithUV((double) i2, 0.0D, -90.0D, 1.0D, 0.0D);
	...
	private void renderFreezeFrame(float brightness, int width, int height) {
		...
		Tessellator tes = Tessellator.instance;
		tes.startDrawingQuads();
		tes.addVertexWithUV(0.0D, (double) height, -90.0D, 0.0D, 1.0D);
		tes.addVertexWithUV((double) width, (double) height, -90.0D, 1.0D, 1.0D);
		tes.addVertexWithUV((double) width, 0.0D, -90.0D, 1.0D, 0.0D);
		tes.addVertexWithUV(0.0D, 0.0D, -90.0D, 0.0D, 0.0D);
		tes.draw();
		GL11.glDepthMask(true);
		GL11.glEnable(GL11.GL_DEPTH_TEST);
	...
	private void renderPortalOverlay(float f1, int i2, int i3) {
		...
		/*
		 * float f4 = (float)(Block.portal.blockIndexInTexture % 16) / 16.0F; float f5 =
		 * (float)(Block.portal.blockIndexInTexture / 16) / 16.0F; float f6 =
		 * (float)(Block.portal.blockIndexInTexture % 16 + 1) / 16.0F; float f7 =
		 * (float)(Block.portal.blockIndexInTexture / 16 + 1) / 16.0F;
		 */
		Idx2uvF.calc(Block.portal.blockIndexInTexture);
		double f4 = Idx2uvF.u1;
		double f5 = Idx2uvF.v1;
		double f6 = Idx2uvF.u2;
	...
	public void addChatMessage(String string1) {
		while (this.mc.fontRenderer.getStringWidth(string1) > 320) {
			int i2;
			for (i2 = 1; i2 < string1.length()
					&& this.mc.fontRenderer.getStringWidth(string1.substring(0, i2 + 1)) <= 320; ++i2) {
			}
