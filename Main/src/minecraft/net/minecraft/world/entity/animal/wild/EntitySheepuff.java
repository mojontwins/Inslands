...
public class EntitySheepuff extends EntityAetherAnimal {
	...
	public boolean interact(EntityPlayer player) {
		...
		if (heldItem != null && heldItem.itemID == Item.shears.shiftedIndex && !this.getSheared()) {
			if (!this.worldObj.isRemote) {
				...
				if (this.getPuffed()) {
					...
					value = 2 + this.rand.nextInt(3);

					for (i = 0; i < value; ++i) {
						item = this.entityDropItem(new ItemStack(Block.cloth.blockID, 1, this.getFleeceColor()), 1.0F);
		...
		if (heldItem != null && heldItem.itemID == Item.dyePowder.shiftedIndex && !this.getSheared()) {
			value = BlockCloth.getDyeFromBlock(heldItem.getItemDamage());
			if (this.getFleeceColor() != value) {
				if (this.getPuffed() && heldItem.stackSize >= 2) {
					this.setFleeceColor(value);
	...
	public void setPuffed(boolean puffed) {
		byte dataByte = this.dataWatcher.getWatchableObjectByte(16);
		if (puffed) {
			this.dataWatcher.updateObject(16, (byte) (dataByte | 32));
		} else {
			this.dataWatcher.updateObject(16, (byte) (dataByte & -33));
		}
	}

	public static int getRandomFleeceColor(Random random) {
		int roll = random.nextInt(100);
		return roll < 5 ? 3 : (roll < 10 ? 9 : (roll < 15 ? 5 : (roll < 18 ? 6 : (random.nextInt(500) != 0 ? 0 : 10))));
	}
}
