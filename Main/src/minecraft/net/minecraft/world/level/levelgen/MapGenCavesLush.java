...
public class MapGenCavesLush extends MapGenCaves {
	...
	private void caveTop(World world, int i2, int i3, int chunkX, int chunkZ, byte[] data) {
		...
		for(int i8 = 0; i8 < i7; ++i8) {
			...
			double y = (double)(64 + this.rand.nextInt(64));
			double z = (double)(i3 * 16 + this.rand.nextInt(16));
			
			int i15 = 1;
						
			if(this.rand.nextInt(16) == 0) {
				this.generateLargeCaveNodeUpper(this.rand.nextLong(), chunkX, chunkZ, data, x, y, z);
				i15 += this.rand.nextInt(4);
			}

			for(int i16 = 0; i16 < i15; ++i16) {
				float yaw = this.rand.nextFloat() * (float)Math.PI * 2.0F;
				float pitch = (this.rand.nextFloat() - 0.5F) * 2.0F / 4.0F;
				float nodeSize = this.rand.nextFloat() * 2.0F + this.rand.nextFloat();
				this.generateCaveNodeUpper(this.rand.nextLong(), chunkX, chunkZ, data, x, y, z, nodeSize, yaw, pitch, 0, 0, 1.0D);
			}
		}
		
	}

	private void caveLow(World world, int i2, int i3, int chunkX, int chunkZ, byte[] data) {
		int i7 = this.rand.nextInt(this.rand.nextInt(this.rand.nextInt(40) + 1) + 1);
		if(this.rand.nextInt(31) != 0) {
			i7 = 0;
		}

		for(int i8 = 0; i8 < i7; ++i8) {
			double x = (double)(i2 * 16 + this.rand.nextInt(16));
			double y = (double)(this.rand.nextInt(8 + this.rand.nextInt(120)));
			double z = (double)(i3 * 16 + this.rand.nextInt(16));
	...
	protected void generateLargeCaveNodeUpper (long longSeed, int chunkX, int chunkZ, byte[] data, double x, double y, double z) {
		this.generateCaveNodeUpper(longSeed, chunkX, chunkZ, data, x, y, z, 1.0F + this.rand.nextFloat() * 5.0F, 0.0F, 0.0F, -1, -1, 0.5D); // 2.0 was 1.0
	}
	
	protected void generateCaveNodeUpper (long longSeed, int chunkX, int chunkZ, byte[] data, double x, double y, double z, float nodeSize, float yaw, float pitch, int i13, int i14, double horzVertRatio) {
		...
		for(; i13 < i14; ++i13) {
			...
			if(z52 || random23.nextInt(4) != 0) {
				...
				if(x >= chunkCenterX - 16.0D - amplitudeHorz * 2.0D && z >= chunkCenterZ - 16.0D - amplitudeHorz * 2.0D && x <= chunkCenterX + 16.0D + amplitudeHorz * 2.0D && z <= chunkCenterZ + 16.0D + amplitudeHorz * 2.0D) {
					...
					// Must generate a floor made of grass
