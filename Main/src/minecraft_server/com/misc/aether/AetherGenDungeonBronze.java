...
public class AetherGenDungeonBronze extends AetherGenBuildings {
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		if(this.isBoxSolid(world, i, j, k, 16, 12, 16) && this.isBoxSolid(world, i + 20, j, k + 2, 12, 12, 12)) {
			...
			if(!this.isBoxSolidRelaxed(world, x, j, z, 12, 12, 12)) {
				System.out.println ("Simple Bronze Dungeon @ " + i + " " + j + " " + k);
				return true;
			} else {
	...
	public boolean generateNextRoom(World world, Random random, int i, int j, int k) {
		...
		if(this.n > this.numRooms && !this.finished) {
			...
			if(!this.isBoxSolidRelaxed(world, x, j, z, 12, 8, 12)) {
				return false;
			} else {
				...
				for(type = x + 2; type < x + 10; type += 7) {
					for(p = z + 2; p < z + 10; p += 7) {
						if (!world.isAirBlock(type, j, p)) world.setBlockAndMetadata(type, j, p, Block.trap.blockID, 0);
					}
				}
	...
	private ItemStack getNormalLoot(Random random) {
		...
		case 9:
			...
			break;
		case 10:
			return new ItemStack(Block.cryingObsidian);
		case 11:
	...
	private ItemStack getBronzeLoot(Random random) {
		...
		case 5:
			return new ItemStack(Item.saddle,random.nextInt(16));
		case 6:
			return new ItemStack(Item.diamond, random.nextInt(16));
		default:
			return new ItemStack(Item.cryingObsidianWand, random.nextInt(16));
		}
	}
}
