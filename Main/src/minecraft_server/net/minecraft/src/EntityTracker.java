...
public class EntityTracker {
	...
	public void trackEntity(Entity entity1) {
		...
		} else if(entity1 instanceof EntityThrowablePotion) {
			this.trackEntity(entity1, 64, 20, false);
	...
	public void trackEntity(Entity entity1, int trackingDistanceThreshold, int updateFrequency) {
		this.trackEntity(entity1, trackingDistanceThreshold, updateFrequency, false);
	}

	public void trackEntity(Entity entity1, int trackingDistanceThreshold, int updateFrequency, boolean shouldSendMotionUpdates) {
		...
		if(this.trackedEntityHashTable.containsItem(entity1.entityId)) {
			...
			EntityTrackerEntry entityTrackerEntry5 = new EntityTrackerEntry(entity1, trackingDistanceThreshold, updateFrequency, shouldSendMotionUpdates);
			this.trackedEntitySet.add(entityTrackerEntry5);
			this.trackedEntityHashTable.addKey(entity1.entityId, entityTrackerEntry5);
			entityTrackerEntry5.updatePlayerEntities(this.mcServer.getWorldManager(this.dimension).playerEntities);
		}
	}
	...
	public void updateTrackedEntities() {
		...
		while(iterator2.hasNext()) {
			...
			entityTrackerEntry3.updatePlayerList(this.mcServer.getWorldManager(this.dimension).playerEntities);
