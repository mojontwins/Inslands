...
public class ChunkProviderCaves extends ChunkProviderGenerate {
	...
	public Chunk provideChunk(int chunkX, int chunkZ) {
		if(chunkX < 1 || chunkX >= WorldSize.xChunks - 1 || chunkZ < 1 || chunkZ >= WorldSize.zChunks - 1) {
			byte blockArray[] = new byte[32768];
			Chunk chunk = new Chunk(this.worldObj, blockArray, new byte[32768], chunkX, chunkZ);
			...
			for(int x = 0; x < 16; x ++) {
				for(int z = 0; z < 16; z ++) {
					int index = x << 11 | z << 7;
					for(int y = 0; y < 128; y ++) {
						byte blockID = (byte)(rand.nextInt(256) == 0 ? Block.glowStone.blockID : (((x < 3 || x > 12 || z < 3 || z > 12) && rand.nextBoolean()) ? 0 : Block.bedrock.blockID));
