...
public class CanvasIsomPreview extends Canvas implements KeyListener, MouseListener, MouseMotionListener, Runnable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 1063392002219056930L;
	...
	private List<IsoImageBuffer> imageBufferList = Collections.synchronizedList(new LinkedList<IsoImageBuffer>());
	...
	public File getAppDir(String string1) {
		...
		File file3;
		switch(CanvasIsomPreview.SyntheticClass_1.$SwitchMap$net$minecraft$src$EnumOS1[getOs().ordinal()]) {
		case 1:
		case 2:
		...
		synchronized(this.imageBufferList) {
			...
			for(int i3 = 0; i3 < 64; ++i3) {
				for(int i4 = 0; i4 < 64; ++i4) {
					this.imageBuffers[i3][i4].init(this.worldObj, i3, i4);
				}
			}
	...
	public void run() {
		...
		while(this.running) {
			...
			if(isoImageBuffer2 != null) {
				if(this.field_1793_a - isoImageBuffer2.lastVisible < 2) {
	...
	public void drawScreen(Graphics2D graphics2D1) {
		...
		for(i10 = i8; i10 <= i9; ++i10) {
			for(int i11 = i6; i11 <= i7; ++i11) {
				...
				isoImageBuffer14.lastVisible = this.field_1793_a;
	...
	static final class SyntheticClass_1 {
		static final int[] $SwitchMap$net$minecraft$src$EnumOS1 = new int[EnumOS1.values().length];
