...
public class EntitySlider extends EntityFlying implements IAetherBoss {
	...
	public EntitySlider(World world) {
		...
		this.health = 500;
		this.dennis = 1;
		this.texture = "/aether/mobs/sliderSleep.png";
		this.chatTime = 60;
	...
	public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
		...
		if(this.awake) {
			if(this.criticalCondition()) {
				this.texture = "/aether/mobs/sliderAwake_red.png";
			} else {
				this.texture = "/aether/mobs/sliderAwake.png";
			}
		}
	...
	protected void dropFewItems() {
		...
		this.entityDropItem(new ItemStack(AetherItems.Key, 1, 0), 0.0F);
	}
		...
		if(e1 != null && e1 instanceof EntityPlayer) {
			...
			if(stack != null && stack.getItem() != null) {
				if(stack.getItem() instanceof Tool) {
					Tool flag = (Tool)stack.getItem();
					if(!flag.canHarvest(Block.stone)) {
							...
							if(this.health <= 0) {
								...
								while(true) {
									...
									for(int c = this.dungeonZ + 5; c < this.dungeonZ + 11; ++c) {
										this.worldObj.setBlock(a, i15, c, AetherBlocks.LockedDungeonStone.blockID);
									}
	...
	private void unlockBlock(int i, int j, int k) {
		...
		if(id == AetherBlocks.LockedDungeonStone.blockID) {
			this.worldObj.setBlockAndMetadata(i, j, k, AetherBlocks.DungeonStone.blockID, this.worldObj.getBlockMetadata(i, j, k));
		...
		if(id == AetherBlocks.LockedLightDungeonStone.blockID) {
			this.worldObj.setBlockAndMetadata(i, j, k, AetherBlocks.LightDungeonStone.blockID, this.worldObj.getBlockMetadata(i, j, k));
			this.unlockBlock(i + 1, j, k);
			this.unlockBlock(i - 1, j, k);
			this.unlockBlock(i, j + 1, k);
			this.unlockBlock(i, j - 1, k);
	...
	public void blockCrush(int x, int y, int z) {
		...
		if(a != 0 && a != AetherBlocks.LockedDungeonStone.blockID && a != AetherBlocks.LockedLightDungeonStone.blockID) {
