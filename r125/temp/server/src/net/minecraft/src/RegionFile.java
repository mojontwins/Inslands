...
public class RegionFile {
					...
					if(i4 + i5 > this.sectorFree.size()) {
						...
						if(i6 > 4096 * i5) {
							...
							if(b7 == 1) {
								...
								dataInputStream9 = new DataInputStream(new BufferedInputStream(new GZIPInputStream(new ByteArrayInputStream(b8))));
								return dataInputStream9;
							} else if(b7 == 2) {
								b8 = new byte[i6 - 1];
								this.dataFile.read(b8);
								dataInputStream9 = new DataInputStream(new BufferedInputStream(new InflaterInputStream(new ByteArrayInputStream(b8))));
								return dataInputStream9;
							} else {
	...
	protected synchronized void write(int i1, int i2, byte[] b3, int i4) {
		try {
			...
			this.setChunkTimestamp(i1, i2, (int)(System.currentTimeMillis() / 1000L));
	...
	public boolean isChunkSaved(int i1, int i2) {
	...
	private void setChunkTimestamp(int i1, int i2, int i3) throws IOException {
