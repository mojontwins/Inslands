...
public class RenderGlobal implements IWorldAccess {
	...
	public void renderEntities(Vec3D vec3D1, ICamera iCamera2, float f3) {
		if(this.renderEntitiesStartupCounter > 0) {
			...
			for(i6 = 0; i6 < list5.size(); ++i6) {
				...
				if(entity7.isInRangeToRenderVec3D(vec3D1) && (entity7.ignoreFrustumCheck || iCamera2.isBoundingBoxInFrustum(entity7.boundingBox)) && (entity7 != this.mc.renderViewEntity || this.mc.gameSettings.thirdPersonView != 0 || this.mc.renderViewEntity.isPlayerSleeping()) && this.worldObj.blockExists(MathHelper.floor_double(entity7.posX), 0, MathHelper.floor_double(entity7.posZ))) {
					++this.countEntitiesRendered;
					RenderManager.instance.renderEntity(entity7, f3);
				}
			}
	...
	public void renderSky(float f1) {
		if(this.mc.theWorld.worldProvider.worldType == 1) {
			...
			double d23 = this.mc.thePlayer.getPosition(f1).yCoord - this.worldObj.getSeaLevel();
			if(d23 < 0.0D) {
				GL11.glPushMatrix();
	...
	public EntityFX func_40193_b(String string1, double d2, double d4, double d6, double d8, double d10, double d12) {
