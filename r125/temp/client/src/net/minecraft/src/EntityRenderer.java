...
public class EntityRenderer {
	...
	public void renderWorld(float f1, long j2) {
		...
		IChunkProvider iChunkProvider13 = this.mc.theWorld.getChunkProvider();
		int i16;
		...
		for(int i18 = 0; i18 < 2; ++i18) {
			...
			if(this.debugViewDirection == 0) {
				...
				this.disableLightmap((double)f1);
				if(this.mc.objectMouseOver != null && entityLiving4.isInsideOfMaterial(Material.water) && entityLiving4 instanceof EntityPlayer && !this.mc.gameSettings.hideGUI) {
			...
			GL11.glEnable(GL11.GL_CULL_FACE);
			GL11.glDisable(GL11.GL_BLEND);
			if(this.cameraZoom == 1.0D && entityLiving4 instanceof EntityPlayer && !this.mc.gameSettings.hideGUI && this.mc.objectMouseOver != null && !entityLiving4.isInsideOfMaterial(Material.water)) {
				entityPlayer21 = (EntityPlayer)entityLiving4;
				GL11.glDisable(GL11.GL_ALPHA_TEST);
				Profiler.endStartSection("outline");
				renderGlobal5.drawBlockBreaking(entityPlayer21, this.mc.objectMouseOver, 0, entityPlayer21.inventory.getCurrentItem(), f1);
