...
public class EffectRenderer {
		...
		for(int i8 = 0; i8 < 3; ++i8) {
			if(this.fxLayers[i8].size() != 0) {
				...
				for(int i11 = 0; i11 < this.fxLayers[i8].size(); ++i11) {
					...
					tessellator10.setBrightness(entityFX12.getBrightnessForRender(f2));
	...
	public void func_1187_b(Entity entity1, float f2) {
		...
		byte b9 = 3;
		if(this.fxLayers[b9].size() != 0) {
			Tessellator tessellator10 = Tessellator.instance;

			for(int i11 = 0; i11 < this.fxLayers[b9].size(); ++i11) {
				EntityFX entityFX12 = (EntityFX)this.fxLayers[b9].get(i11);
				tessellator10.setBrightness(entityFX12.getBrightnessForRender(f2));
