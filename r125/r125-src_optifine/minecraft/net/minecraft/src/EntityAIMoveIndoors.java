...
public class EntityAIMoveIndoors extends EntityAIBase {
	...
	public void startExecuting() {
		...
		if(this.entityObj.getDistanceSq((double)this.doorInfo.getInsidePosX(), (double)this.doorInfo.posY, (double)this.doorInfo.getInsidePosZ()) > 256.0D) {
			Vec3D vec3D1 = RandomPositionGenerator.func_48620_a(this.entityObj, 14, 3, Vec3D.createVector((double)this.doorInfo.getInsidePosX() + 0.5D, (double)this.doorInfo.getInsidePosY(), (double)this.doorInfo.getInsidePosZ() + 0.5D));
			if(vec3D1 != null) {
				this.entityObj.getNavigator().func_48666_a(vec3D1.xCoord, vec3D1.yCoord, vec3D1.zCoord, 0.3F);
			}
		} else {
