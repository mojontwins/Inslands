...
public class TextureHDCompassFX extends TextureFX implements TextureHDFX {
	...
	private void setup() {
		...
		try {
			...
			ioexception.getRGB(x, y, this.tileWidth, this.tileWidth, this.compassIconImageData, 0, this.tileWidth);
	...
	public void onTick() {
		int i10000 = this.tileWidth * this.tileWidth;
		...
		if(this.mc.theWorld != null && this.mc.thePlayer != null) {
			ChunkCoordinates d1 = this.mc.theWorld.getSpawnPoint();
			double d2 = (double)d1.posX - this.mc.thePlayer.posX;
			double d4 = (double)d1.posZ - this.mc.thePlayer.posZ;
			d = (double)(this.mc.thePlayer.rotationYaw - 90.0F) * Math.PI / 180.0D - Math.atan2(d4, d2);
		...
		double d27;
		for(d27 = d - this.showAngle; d27 < -3.141592653589793D; d27 += Math.PI * 2D) {
		}
