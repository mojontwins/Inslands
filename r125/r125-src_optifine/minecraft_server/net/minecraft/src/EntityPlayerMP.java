...
public class EntityPlayerMP extends EntityPlayer implements ICrafting {
	...
	public void onUpdateEntity(boolean z1) {
				...
				if(z14) {
					...
					if(worldServer15.blockExists(chunkCoordIntPair10.chunkXPos << 4, 0, chunkCoordIntPair10.chunkZPos << 4)) {
						...
						if(chunk16.isTerrainPopulated) {
							...
							List list8 = worldServer15.getTileEntityList(chunkCoordIntPair10.chunkXPos * 16, 0, chunkCoordIntPair10.chunkZPos * 16, chunkCoordIntPair10.chunkXPos * 16 + 16, 256, chunkCoordIntPair10.chunkZPos * 16 + 16);
		...
		if(this.getHealth() != this.lastHealth || this.field_35221_cc != this.foodStats.getFoodLevel() || this.foodStats.getSaturationLevel() == 0.0F != this.field_35222_cd) {
	...
	public void displayGUIEnchantment(int i1, int i2, int i3) {
		...
		this.craftingInventory = new ContainerEnchantment(this.inventory, this.worldObj, i1, i2, i3);
		this.craftingInventory.windowId = this.currentWindowId;
		this.craftingInventory.onCraftGuiOpened(this);
	}
	...
	public void displayGUIDispenser(TileEntityDispenser tileEntityDispenser1) {
		...
		this.craftingInventory = new ContainerDispenser(this.inventory, tileEntityDispenser1);
		this.craftingInventory.windowId = this.currentWindowId;
		this.craftingInventory.onCraftGuiOpened(this);
	}
	...
	public void displayGUIBrewingStand(TileEntityBrewingStand tileEntityBrewingStand1) {
		this.getNextWidowId();
		this.playerNetServerHandler.sendPacket(new Packet100OpenWindow(this.currentWindowId, 5, tileEntityBrewingStand1.getInvName(), tileEntityBrewingStand1.getSizeInventory()));
		this.craftingInventory = new ContainerBrewingStand(this.inventory, tileEntityBrewingStand1);
		this.craftingInventory.windowId = this.currentWindowId;
		this.craftingInventory.onCraftGuiOpened(this);
	}
	...
	protected void onNewPotionEffect(PotionEffect potionEffect1) {
		super.onNewPotionEffect(potionEffect1);
	...
	protected void onChangedPotionEffect(PotionEffect potionEffect1) {
		super.onChangedPotionEffect(potionEffect1);
		this.playerNetServerHandler.sendPacket(new Packet41EntityEffect(this.entityId, potionEffect1));
	}

	protected void onFinishedPotionEffect(PotionEffect potionEffect1) {
		super.onFinishedPotionEffect(potionEffect1);
