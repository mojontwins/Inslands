...
public class EntityEnderEye extends Entity {
	...
	public void func_40056_a(double d1, int i3, double d4) {
		double d6 = d1 - this.posX;
		double d8 = d4 - this.posZ;
		float f10 = MathHelper.sqrt_double(d6 * d6 + d8 * d8);
		if(f10 > 12.0F) {
			this.field_40060_b = this.posX + d6 / (double)f10 * 12.0D;
			this.field_40058_d = this.posZ + d8 / (double)f10 * 12.0D;
			this.field_40061_c = this.posY + 8.0D;
		} else {
			this.field_40060_b = d1;
			this.field_40061_c = (double)i3;
			this.field_40058_d = d4;
		}
	...
	public void onUpdate() {
		...
		if(!this.worldObj.isRemote) {
			double d2 = this.field_40060_b - this.posX;
			double d4 = this.field_40058_d - this.posZ;
