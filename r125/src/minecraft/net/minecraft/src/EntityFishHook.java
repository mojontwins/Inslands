...
public class EntityFishHook extends Entity {
	...
	public EntityFishHook(World world1) {
		super(world1);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
	...
	public EntityFishHook(World world1, double d2, double d4, double d6) {
		this(world1);
		this.setPosition(d2, d4, d6);
		this.ignoreFrustumCheck = true;
	}

	public EntityFishHook(World world1, EntityPlayer entityPlayer2) {
		super(world1);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
	...
	public void calculateVelocity(double d1, double d3, double d5, float f7, float f8) {
	...
	public void setPositionAndRotation2(double d1, double d3, double d5, float f7, float f8, int i9) {
		...
		this.fishY = d3;
		this.fishZ = d5;
		this.fishYaw = (double)f7;
		this.fishPitch = (double)f8;
		this.fishPosRotationIncrements = i9;
	...
	public void onUpdate() {
		...
		if(this.fishPosRotationIncrements > 0) {
			double d21 = this.posX + (this.fishX - this.posX) / (double)this.fishPosRotationIncrements;
			double d22 = this.posY + (this.fishY - this.posY) / (double)this.fishPosRotationIncrements;
			double d23 = this.posZ + (this.fishZ - this.posZ) / (double)this.fishPosRotationIncrements;

			double d7;
			for(d7 = this.fishYaw - (double)this.rotationYaw; d7 < -180.0D; d7 += 360.0D) {
			}
			...
			this.rotationYaw = (float)((double)this.rotationYaw + d7 / (double)this.fishPosRotationIncrements);
			...
			if(movingObjectPosition3 != null) {
				if(movingObjectPosition3.entityHit != null) {
					if(movingObjectPosition3.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this, this.angler), 0)) {
