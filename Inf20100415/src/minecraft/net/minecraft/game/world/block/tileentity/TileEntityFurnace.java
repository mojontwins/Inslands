...
public class TileEntityFurnace extends TileEntity implements IInventory {
	...
	public final void updateEntity() {
		...
		if(z1 != this.furnaceBurnTime > 0) {
			...
			world9.setBlockMetadataWithNotify(i8, i4, i5, i6);
	...
	private boolean canSmelt() {
		...
		return this.furnaceItemStacks[0] == null ? false : ((i1 = smeltItem(this.furnaceItemStacks[0].getItem().shiftedIndex)) < 0 ? false : (this.furnaceItemStacks[2] == null ? true : (this.furnaceItemStacks[2].itemID != i1 ? false : (this.furnaceItemStacks[2].stackSize < 64 && this.furnaceItemStacks[2].stackSize < (itemStack2 = this.furnaceItemStacks[2]).getItem().getItemStackLimit() ? true : this.furnaceItemStacks[2].stackSize < Item.itemsList[i1].getItemStackLimit()))));
	}

	private static int smeltItem(int i0) {
		return i0 == Block.oreIron.blockID ? Item.ingotIron.shiftedIndex : (i0 == Block.oreGold.blockID ? Item.ingotGold.shiftedIndex : (i0 == Block.oreDiamond.blockID ? Item.diamod.shiftedIndex : (i0 == Block.sand.blockID ? Block.glass.blockID : (i0 == Item.porkRaw.shiftedIndex ? Item.porkCooked.shiftedIndex : (i0 == Block.cobblestone.blockID ? Block.stone.blockID : -1)))));
	}

	private static int getItemBurnTime(ItemStack itemStack0) {
		int i1;
		return itemStack0 == null ? 0 : ((i1 = itemStack0.getItem().shiftedIndex) < 256 && Block.blocksList[i1].blockMaterial == Material.wood ? 300 : (i1 == Item.stick.shiftedIndex ? 100 : (i1 == Item.coal.shiftedIndex ? 1600 : 0)));
	}
