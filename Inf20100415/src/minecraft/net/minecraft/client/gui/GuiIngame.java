...
public final class GuiIngame extends Gui {
	...
	public final void renderGameOverlay(float f1) {
		...
		for(i10 = 0; i10 < 9; ++i10) {
			...
			if((itemStack22 = this.mc.thePlayer.inventory.mainInventory[i10]) != null) {
				...
				if((f9 = (float)itemStack22.animationsToGo - f1) > 0.0F) {
					GL11.glPushMatrix();
					float f24 = 1.0F + f9 / 5.0F;
					GL11.glTranslatef((float)(i25 + 8), (float)(i21 + 12), 0.0F);
					GL11.glScalef(1.0F / f24, (f24 + 1.0F) / 2.0F, 1.0F);
					GL11.glTranslatef((float)(-(i25 + 8)), (float)(-(i21 + 12)), 0.0F);
				}
		...
		if(this.mc.gameSettings.showFPS) {
			fontRenderer4.drawStringWithShadow("Minecraft Infdev (" + this.mc.debug + ")", 2, 2, 0xFFFFFF);
			...
			minecraft23 = this.mc;
			fontRenderer4.drawStringWithShadow("P: " + minecraft23.effectRenderer.getStatistics() + ". T: " + minecraft23.theWorld.getDebugLoadedEntities(), 2, 32, 0xFFFFFF);
			...
			fontRenderer4.drawStringWithShadow("Minecraft Infdev", 2, 2, 0xFFFFFF);
		}

		for(i12 = 0; i12 < this.chatMessageList.size() && i12 < 10; ++i12) {
			if(((ChatLine)this.chatMessageList.get(i12)).updateCounter < 200) {
				fontRenderer4.drawStringWithShadow(((ChatLine)this.chatMessageList.get(i12)).message, 2, i19 - 8 - i12 * 9 - 20, 0xFFFFFF);
			}
		}
