...
public class RenderEngine {
	...
	public final int getTextureForDownloadableImage(String string1, String string2) {
		...
		if((threadDownloadImageData6 = (ThreadDownloadImageData)this.urlToImageDataMap.get(string1)) != null && threadDownloadImageData6.image != null && !threadDownloadImageData6.textureSetupComplete) {
			if(threadDownloadImageData6.textureIntDownload < 0) {
				...
				this.textureNameToImageMap.put(i5, bufferedImage4);
				threadDownloadImageData6.textureIntDownload = i5;
			} else {
				this.setupTexture(threadDownloadImageData6.image, threadDownloadImageData6.textureIntDownload);
			}

			threadDownloadImageData6.textureSetupComplete = true;
		}

		return threadDownloadImageData6 != null && threadDownloadImageData6.textureIntDownload >= 0 ? threadDownloadImageData6.textureIntDownload : this.getTexture(string2);
	}

	public final ThreadDownloadImageData obtainImageData(String string1, ImageBufferDownload imageBufferDownload2) {
		...
		if((threadDownloadImageData3 = (ThreadDownloadImageData)this.urlToImageDataMap.get(string1)) == null) {
			this.urlToImageDataMap.put(string1, new ThreadDownloadImageData(string1, imageBufferDownload2));
		} else {
	...
	public final void releaseImageData(String string1) {
		...
		if((threadDownloadImageData2 = (ThreadDownloadImageData)this.urlToImageDataMap.get(string1)) != null) {
			...
			if(threadDownloadImageData2.referenceCount == 0) {
				if(threadDownloadImageData2.textureIntDownload >= 0) {
					int i3 = threadDownloadImageData2.textureIntDownload;
					this.textureNameToImageMap.remove(i3);
					this.singleIntBuffer.clear();
	...
	public final void refreshTextures() {
		...
		while(iterator1.hasNext()) {
			...
			bufferedImage3 = (BufferedImage)this.textureNameToImageMap.get(i2);
