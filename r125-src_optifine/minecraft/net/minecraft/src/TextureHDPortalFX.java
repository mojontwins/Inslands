		...
		for(int i = 0; i < 32; ++i) {
			for(int x = 0; x < this.tileWidth; ++x) {
				for(int y = 0; y < this.tileWidth; ++y) {
					...
					int i13 = (int)(f * f * 200.0F + 55.0F);
					int i14 = (int)(f * f * f * f * 255.0F);
					int i15 = (int)(f * 100.0F + 155.0F);
					int i16 = y * this.tileWidth + x;
					this.buffer[i][i16 * 4 + 0] = (byte)i13;
					this.buffer[i][i16 * 4 + 1] = (byte)i14;
					this.buffer[i][i16 * 4 + 2] = (byte)i1;
					this.buffer[i][i16 * 4 + 3] = (byte)i15;
				}
			}
		}
