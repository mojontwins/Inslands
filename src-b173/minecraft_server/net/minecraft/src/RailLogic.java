...
class RailLogic {
	...
	private void updateConnections() {
		for(int i1 = 0; i1 < this.connectedTracks.size(); ++i1) {
			RailLogic railLogic2 = this.getrailLogic((ChunkPosition)this.connectedTracks.get(i1));
	...
	private boolean israil(int i1, int i2, int i3) {
	...
	private RailLogic getrailLogic(ChunkPosition chunkPosition1) {
	...
	private int getAdjacentTracks() {
		int i1 = 0;
		if(this.israil(this.trackX, this.trackY, this.trackZ - 1)) {
			++i1;
		}
		...
		if(this.israil(this.trackX - 1, this.trackY, this.trackZ)) {
			++i1;
		}
	...
	private boolean func_786_c(int i1, int i2, int i3) {
		RailLogic railLogic4 = this.getrailLogic(new ChunkPosition(i1, i2, i3));
		if(railLogic4 == null) {
			return false;
		} else {
			railLogic4.updateConnections();
	...
	public void func_792_a(boolean z1, boolean z2) {
		...
		if(z2 || this.worldObj.getBlockMetadata(this.trackX, this.trackY, this.trackZ) != i8) {
			...
			for(int i9 = 0; i9 < this.connectedTracks.size(); ++i9) {
				RailLogic railLogic10 = this.getrailLogic((ChunkPosition)this.connectedTracks.get(i9));
				if(railLogic10 != null) {
					railLogic10.updateConnections();
